...
管理员在群内发送 @机器人用户名 或 /help 可查看命令菜单，菜单消息在 30 秒后自动删除。启动时会注册仅群管理员可见的 Telegram 命令列表，可在输入 / 时选择命令。

群管理员在群内回复目标成员的消息发送 `/whitelist_add`，或发送 `/whitelist_add 123456789`，即可将该成员加入当前群的白名单。使用 `/whitelist_remove`（回复消息）或 `/whitelist_remove 123456789` 移除。支持 `/whitelist_add@机器人用户名` 等带 Bot 用户名的命令。

当前群管理员和群主默认享有消息白名单，无需配置或手动添加；以当前群身份发言的匿名管理员也默认放行，其他频道身份和关联频道自动转发不享有此豁免。个人身份通过 Telegram 实时查询，撤销管理员后自动失去默认豁免（手动白名单仍有效）；查询失败时按普通消息检查。

白名单成员后续发送的消息跳过转发、敏感关键词及重复消息检查，其他群不受影响；入群验证仍正常执行。手动名单重启后保留，移除手动名单不会取消管理员默认豁免。增删名单默认允许所有当前群管理员和群主；如设置 `TELEGRAM_WHITELIST_ADMIN_IDS` 则仅限指定管理员，必须使用个人身份操作。白名单管理权限独立于私聊中继的 `TELEGRAM_ADMIN_CHAT_ID`。
